...
export function parseXmlImportRows(xml: string, mapping: FieldMapping): ParsedImportRow[] {
  ...
  for (let i = 0; i < items.length; i++) {
    ...
    rows.push({
      ...
      images,
      seoTitle: cell(item, mapping.seoTitle),
      seoDescription: cell(item, mapping.seoDescription),
      seoKeywords: cell(item, mapping.seoKeywords),
      geoTargets: cell(item, mapping.geoTargets),
      aeoAnswerSummary: cell(item, mapping.aeoAnswerSummary),
      aeoFaq: cell(item, mapping.aeoFaq),
      variantOptions,
      raw: item,
      errors,
      warnings: [],
