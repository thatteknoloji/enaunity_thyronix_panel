...
export function parseRowsFromBuffer(
  ...
  for (let i = 0; i < parsed.allRows.length; i++) {
    ...
    rows.push({
      ...
      image: images[0] || "",
      images,
      seoTitle: cell(raw, mapping.seoTitle),
      seoDescription: cell(raw, mapping.seoDescription),
      seoKeywords: cell(raw, mapping.seoKeywords),
      geoTargets: cell(raw, mapping.geoTargets),
      aeoAnswerSummary: cell(raw, mapping.aeoAnswerSummary),
      aeoFaq: cell(raw, mapping.aeoFaq),
      variantOptions,
      raw,
      errors,
      warnings,
