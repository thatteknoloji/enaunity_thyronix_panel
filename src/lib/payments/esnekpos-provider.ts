    ...
    async createPayment(params: CreatePaymentParams): Promise<PaymentResult> {
      ...
      try {
        ...
        if (status !== "SUCCESS" || returnCode !== "0" || !redirectUrl) {
          ...
          const authFailed =
            returnCode === "102" ||
            /authentication\.error|kimlik doğrulama|merchant_key|merchant key/i.test(returnMessage);
          const adminHint = authFailed
            ? " EsnekPOS Merchant ID (enaunity.com.tr/) ve Merchant Key admin panelde yeniden kaydedilmeli — Key alanını boş bırakıp Kaydet yapmayın, key'i tekrar yapıştırın."
            : returnCode === "100" ||
              ...
              : /IP adresinden/i.test(returnMessage)
                ? " Sunucu IP adresi EsnekPOS whitelist'ine eklenmeli."
                : "";
          return {
            success: false,
            status: "FAILED",
