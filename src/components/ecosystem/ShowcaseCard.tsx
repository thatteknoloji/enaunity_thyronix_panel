...
function resolveCardHref(product: ProductShowcaseDTO): string {
  const raw = product.ctaUrl || product.productUrl || `/ecosystem/${product.slug}`;
  if (raw.startsWith("/gateway/") || raw.startsWith("/dealer/")) {
    return `/platform/${product.slug}`;
  }
  return raw;
}
...
export function ShowcaseCard({ product, index = 0, preview }: Props) {
  ...
  const href = resolveCardHref(product);
  ...
  const openInNewTab = product.linkTarget === "_blank" && !isInternal;

  const cardClass =
    "group relative flex h-full flex-col rounded-[1.75rem] p-8 md:p-10 lg:p-12 transition-all duration-500 hover:-translate-y-2";
  ...
  const inner = (
    <>
      ...
      />
      <div className="absolute inset-px rounded-[calc(1.75rem-1px)] bg-ena-card/90 backdrop-blur-md pointer-events-none" />
      ...
      <div className="relative flex flex-col flex-1 pointer-events-none">
    ...
    >
      {preview ? (
        ...
        <Link
          ...
          style={cardStyle}
          aria-label={`${product.name} — ${product.ctaText || "Keşfet"}`}
        >
          {inner}
        </Link>
      )}
