...
import { generateBlueprintUniverse } from "@/lib/page-factory/universe-service";
...
export async function POST(req: Request, { params }: Params) {
  try {
    ...
    const result = await generateBlueprintUniverse(id, body, isAdminRole(user.role));
    return NextResponse.json({ success: true, data: result });
  } catch (e) {
    const msg = e instanceof Error ? e.message : "Blueprint üretilemedi";
    const status = msg.includes("engellendi") ? 400 : 500;
