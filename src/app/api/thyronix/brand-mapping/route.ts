...
export async function GET() {
  try {
    ...
    const items = await prisma.thyronixBrandMapping.findMany({
...
export async function POST(req: Request) {
  try {
    ...
    const item = await prisma.thyronixBrandMapping.create({ data: body });
    const affected = await prisma.thyronixProduct.updateMany({
      where: withTenantFilter(user, {
        brand: body.sourceBrand,
        ...(body.sourceId ? { sourceId: body.sourceId } : {}),
      }),
      data: { brand: body.targetBrand },
    });
    await prisma.thyronixBrandMapping.update({ where: { id: item.id }, data: { affectedCount: affected.count } });
...
export async function DELETE(req: Request) {
  try {
    ...
    const row = await prisma.thyronixBrandMapping.findUnique({ where: { id } });
    ...
    await prisma.thyronixBrandMapping.delete({ where: { id } });
...
export async function PUT(req: Request) {
  try {
    ...
    const item = await prisma.thyronixBrandMapping.update({ where: { id }, data });
    return NextResponse.json({ success: true, data: item });
  } catch (e) {
    return thyronixErrorResponse(e);
  }
